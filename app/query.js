...
var queryChaincode = function(peer, channelName, chaincodeName, fcn, args, username, org) {
    ...
    return helper.getRegisteredUsers(username, org).then((user) => {
        ...
        var request = {
            ...
            fcn: fcn,
            args: args,
            targets: helper.newPeers([helper.getPeerAddressByName(org, peer)])
        };
        ...
        if (response_payloads) {
            for (let i = 0; i < response_payloads.length; i++) {
                logger.info('Key ' + args[0] + ' has ' + response_payloads[i].toString('utf8') +
                    ' after the put');
                return 'Key ' + args[0] + ' has ' + response_payloads[i].toString('utf8') +
                    ' after the put';
            }
        } else {
...
var getInstalledChaincodes = function(peer, type, username, org, channelName) {
    ...
    return helper.getOrgAdmin(org).then((member) => {
        if (type === 'installed') {
            ...
            let channel = helper.getChannelForOrg(org, channelName);
